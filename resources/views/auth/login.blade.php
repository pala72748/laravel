...
<body class="font-body bg-background-500">
    <section class="h-screen flex items-center justify-center">
        <div class="container md:mx-auto h-full flex items-center justify-center">
            <div
                class="max-w-[90%] md:max-w-[75%] lg:max-w-[33%] 2xl:max-w-[40%] w-full flex flex-row items-center bg-white rounded-lg h-auto min-h-[400px] md:min-h-[500px] lg:min-h-[42vw] 2xl:min-h-[30vw] shadow-lg">
                <div class="w-full mx-4 lg:mx-8 flex flex-col justify-around h-full gap-3 py-6">
                    <h1 class="text-3xl text-center">Sign In</h1>
                    @if (session('success'))
                        <span class="text-center py-2 bg-green-500 text-white">{{session('success')}}</span>
                    @endif
                    @if (session('error'))
                        <span class="text-center py-2 bg-red-500 text-white">{{session('error')}}</span>
                    @endif
                    <form class="flex flex-col gap-4" action="{{ route('post.login') }}" method="post">
                        @csrf
                        <div class="flex flex-col gap-3 relative">
                            <label for="email">Email</label>
                            <input class="border border-background-500 ps-8 py-2 outline-none rounded-full"
                                placeholder="Enter your email" type="email" name="email" id="username">
                    ...
                    <p class="text-center">Don't have an account? <a href="{{ route('register') }}"
                            class="text-background-500">Sign Up</a>
                    </p>
